...
///
/// `serve` starts the target app (`project set-start`) for each replay; without
/// it a backend test with nothing listening scores every run `blocked`, which
/// is excluded from the denominator and so reports "inconclusive" forever.
pub async fn flaky(
    root: &Path,
    id: &str,
    runs: usize,
...
pub async fn flaky_report(
    ...
    let report = flaky(root, id, runs, model, serve).await?;
    ...
    if report.blocked > 0 {
        line.push_str(&format!(
            " [{} blocked/inconclusive run(s) excluded]",
            report.blocked
        ));
    }
    println!("{line}");
